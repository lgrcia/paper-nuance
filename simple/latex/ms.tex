...
\begin{figure}[H]
    \begin{centering}
        \gridline{\fig{../figures/principle_periodic_0.pdf}{0.72\linewidth}{\vspace{-0.5cm}(a) $\ln \mathcal{L}$}}
        \vspace{-0.6cm}
...
\begin{figure}[H]
    \begin{centering}
        \includegraphics[width=0.65\linewidth]{../figures/principle_Q.pdf}
        \caption{}
...
This paper relies on simulated light curves featuring transit signals, instrumental signals and correlated noises such as stellar variability. Let such a flux $f$ of a star simulated at time $t$ be sampled and aranged in the $(1\times N)$ column-vector $\bm{f}$ associated to the column-vector of times $\bm{t}$. An example of such signal is simulated and shown in \autoref{fig:principle_dataset}.
...
\begin{figure}[H]
    ...
    \end{centering}
\end{figure}
In what follows, we describe the models used to simulate the components of this signal, i.e. the periodic transit $\mathcal{T}$, the instrumental systematics $\mathcal{S}$ and the correlated noise $\mathcal{V}$, such that
$$
    f(t) = \textcolor{gray}{\mathcal{T}}(t) %+ \textcolor{Gray}{\mathcal{S}}(t) + \textcolor{Gray}{\mathcal{V}}(t)
$$
computed for $N=2880$ times $t$ corresponding to an observation of 4 days with an exposure time of 2 minutes. 
\subsection{Transit signal}
...
\begin{figure}[H]
    ...
    \end{centering}
\end{figure}
In our study, we simulate all transit signals with $c=12$. The periodic transit signal $\mathcal{T}$ seen in \autoref{fig:simu} corresponds to $\mathcal{T} = \mathcal{T}_{c=12}(t, P=0.7, T_0=0.2, D=0.05)$, all parameters in unit of \textit{days}.

\subsection{Instrumental signals}
...
$$
    \mathcal{S} \sim \bm{\mathcal{S}} = \bm{w}\bm{X}
$$
where the vector $\bm{w}$ are linear coefficients of the model. The simulated flux shown in \autoref{fig:simu} contains a linear model where the $M=4$ columns of the design matrix $\bm{X}$ are given by $\bm{X}_{i} = \bm{t}^i$ (i.e. $\bm{X}$ is the Vandermonde matrix order $3$ of time $t$) and $\bm{w} = [1.0\quad0.0005\quad\min0.0002\quad-0.0005].$

\subsection{Correlated noise}
We generate stellar variability thanks to the \texttt{tinygp}\footnote{\href{https://github.com/dfm/tinygp}{https://github.com/dfm/tinygp}} Python package, providing an implententation of the scalable GP method from \citealt{celerite2} powered by 
\texttt{JAX}\footnote{\href{https://github.com/google/jax}{https://github.com/google/jax}}.
